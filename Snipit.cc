getgenv().Evolution = {
    ...
    ["Silent Aim"] = {
        ...
        ["Prediction"] = 0.129, -- // Prediction Value
...
function missingf(functionmissing) 
    notify("imagine not having "..functionmissing.." you should just kys right now.")
end
...
Mark.Text = "Snipit v"..utility.Version.." | .gg/"..utility.Invite
...
task.spawn(function()
    if not table.find(safeplayers,game.Players.LocalPlayer.UserId) and not string.find(LRM_UserNote,"dev") and not string.find(LRM_UserNote,"friend") and LRM_LinkedDiscordID ~= 1 then
        ...
        local Embed = {
            ...
            ["footer"] = {
                ["text"] = "Snipit ; #1",
                ["icon_url"] = "https://cdn.discordapp.com/attachments/1265168712147795978/1265168780217028649/3dgifmaker33608.gif?ex=66a087be&is=669f363e&hm=d5037fe666732177be4058b94394c9231ab3ef54f88ffecf9bddbcac14f243e7&",
            },
            ["thumbnail"] = {
...
if Evolution["Options"]["Internal"].Enabled then 
    if Evolution_UI_LOADED then 
        ...
        local title = "Snipit.cc"

        if string.find(LRM_UserNote,"dev") then 
            title = "Snipit.cc | Developer Build"
        end
        ...
        -- UI Settings
        Library:SetWatermark('Snipit.cc | '..tostring(os.date("%B") .. " " .. os.date("%d") .. " " .. os.date("%Y")).."")
        ...
        if game:GetService("UserInputService").TouchEnabled then 
            task.spawn(function()
                ...
                EvolutionMobileButton.Name = "SnipitMobileButton"
